...
<template>
  <div class="lg:p-32 lg:pt-0 pt-0 p-10">
    <div
      class="relative w-full h-screen md:h-[70vh] rounded-2xl bg-cover bg-center flex flex-col justify-center items-center text-white text-center px-4 sm:px-6 lg:px-8"
    >
      <div class="absolute inset-0 z-10 bg-black/30 rounded-2xl"></div>
      <ImgPb
        v-if="imgAccueil"
        :record="imgAccueil"
      ...
      />
      <section
        class="flex flex-col justify-center items-center relative z-20 max-w-full px-4 sm:px-6"
      >
        <h1
          class="font-permanent-marker text-xl sm:text-2xl md:text-3xl lg:text-4xl mb-3 sm:mb-4 leading-tight"
        >
          Parce qu'être étudiant c'est déjà un sport
        </h1>
        <p
          class="font-agrandir text-base sm:text-lg md:text-xl max-w-2xl mb-6 sm:mb-8 leading-relaxed"
        >
        ...
        <RouterLink
          to="/authPage"
          class="mt-4 sm:mt-6 bg-white text-black font-bold py-2 sm:py-3 px-5 sm:px-6 rounded-full hover:bg-gray-200 transition inline-block text-sm sm:text-base"
        >
