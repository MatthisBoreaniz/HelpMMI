...
// --- Auth utilisateur ---
const { currentUser, ensureAuthReady,  refreshUser,} = useAuth()
...
const toggleFavorite = async (aideId: string) => {
  ...
  const newIds = isFav ? currentIds.filter((id) => id !== aideId) : [...currentIds, aideId]
      ...
      <div class="p-6 pt-0">
        <section class="rounded-2xl bg-Bleu text-Blanc lg:mr-4 my-4 p-6 ml-0">
          <div class="flex flex-col justify-center items-center text-center mb-6">
            <img
              src="/src/assets/Img/Logo-simple.svg"
              alt="HelpMMI Logo"
